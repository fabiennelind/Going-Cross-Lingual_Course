...
articles_hits <- read.csv("https://raw.githubusercontent.com/fabiennelind/Going-Cross-Lingual_Course/main/data/climate_news_d_annotated.csv")
...
articles_switzerland_de <- subset(articles_hits, Country == "Switzerland" & Language == "de")
articles_switzerland_fr <- subset(articles_hits, Country == "Switzerland" & Language == "fr")
...
library(dplyr)
intercoder_set_switzerland_de <- sample_n(articles_switzerland_de, 2) #select 2 random rows
intercoder_set_switzerland_fr <- sample_n(articles_switzerland_fr, 2) #select 2 random rows
intercoder_set_uk_en <- sample_n(articles_uk_en, 2) #select 2 random rows
intercoder_set <- bind_rows(intercoder_set_switzerland_de, intercoder_set_switzerland_fr, intercoder_set_uk_en)
...
We pick an equal amount of texts per case and language randomly. For this example, we just pick 30 articles from Switzerland (15 German, 15 French) and 15 articles from the UK (English).
...
manual_set_switzerland_de <- sample_n(articles_switzerland_de, 15)
manual_set_switzerland_fr <- sample_n(articles_switzerland_fr, 15)
manual_set_uk_en <- sample_n(articles_uk_en, 15)
manual_set <- bind_rows(manual_set_switzerland_de, manual_set_switzerland_fr, manual_set_uk_en)
